    ...
    <div class="w-full mt-12">
        <div class="w-full my-6 pr-0 lg:pr-2">
            ...
            <div class="leading-loose">
                <form class="w-full p-10" action="{{ route('admin.gambar.update', $gambar->id) }}" method="POST" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    <div class="">
                        <label class="block text-sm text-gray-600" for="title">Judul</label>
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="title" name="title" value="{{$gambar->title}}" type="text" required="" placeholder="Gambar" aria-label="Name">
                    </div>
                    <div class="">
                        <label class="block text-sm text-gray-600" for="content">Isi</label>
                        <textarea  class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" name="content" id="content" cols="" rows="5" placeholder="Isi">{{$gambar->content}}</textarea>
                    </div>

    ...
    <div class="w-full mt-12">
        <div class="w-full my-6 pr-0 lg:pr-2">
            <p class="text-xl pb-6 flex items-center">
                <i class="fas fa-list mr-3"></i> Video Edit
            </p>
            <div class="leading-loose">
                <form class="w-full p-10" action="{{ route('admin.video.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    <div class="">
                        <label class="block text-sm text-gray-600" for="title">Judul</label>
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="title" name="title" value="{{$video->title}}" type="text" required="" placeholder="Gambar" aria-label="Name">
                    </div>
                    <div class="">
                        <label class="block text-sm text-gray-600" for="content">Isi</label>
                        <textarea  class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" name="content" id="content" cols="" rows="5" placeholder="Isi">{{$video->content}}</textarea>
                    </div>
                    <div class="grid flex justify-between grid-cols-1 gap-4 my-3">
                        ...
                        <img class="w-56 h-20 object-cover object-center" src="/public/image/{{ $video->image }}" alt="">
                    ...
                    <div class="mt-6">
                        ...
                        <button type="submit" class="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded" type="submit">Ubah</button>
                    </div>
                </form>
            </div>
        </div>
